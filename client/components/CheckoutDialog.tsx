...
export default function CheckoutDialog({ open, onClose }: CheckoutDialogProps) {
  ...
  // If settings are not loaded, don't render the dialog
  if (!settings) {
    return null;
  }
  ...
  // Get delivery area names based on language
  ...
  const pickupAddress =
    language === "ar" ? pickupAddressAr : pickupAddressEn;
  ...
  const orderMessages = {
    ...
    headline:
      language === "ar"
        ? successHeadlineAr || t("orderSuccess.headlineAr")
        : successHeadlineEn || t("orderSuccess.headline"),
    subtext:
      language === "ar"
        ? successSubtextAr ||
